...
Spiking neural networks (SNNs) are computational models inspired by this principle. In an SNN, neurons do not transmit continuous activations at every step, as in standard artificial neural networks. Instead, they integrate incoming signals over time and emit a spike only when their internal state reaches a threshold [@Gerstner2002; @Neftci2019]. This event-driven mode of computation makes SNNs especially well suited for temporal and sparse data, such as event-camera recordings or neural spike trains.
It also makes them attractive for neuromorphic computing and for modeling biological neural circuits more faithfully.
...
DVS Gesture is recorded with an event-based camera [@Amir2017]. Unlike a conventional camera, which records full image frames at fixed intervals, a DVS camera asynchronously generates an event when the change in local log intensity exceeds a threshold. Each event contains a spatial position, a timestamp, and a polarity: $(x, y, t, \mathrm{polarity})$.
...
For this tutorial, an 8-class subset/labeling derived from the MC_Maze conditions is used. For each trial, spike times are extracted from the 182 recorded neural units and converted into a 1500 ms input sequence aligned to movement onset before being sent to NEST.

:::{figure} static/fig5_mc_maze_input.png
:label: fig-mc-maze-input
:align: center
MC_Maze task context and neural input representation for native NEST e-prop. (a) An example reaching trial provides behavioral context, including the hand trajectory, cursor trajectory, and target location. Task timing signals are used to align trials and define the analysis window used for classification. (b) Neural population activity is represented as a spike raster across the 182 recorded units. (c) The final model input is a 1500 ms spike raster sent to NEST.
The behavioral trajectory and velocity plots provide task context, whereas the neural raster is the actual input used by the model.
:::
...
The training scripts assume that the corresponding datasets are available at the paths documented in their headers. In particular, the MC_Maze script expects preprocessed, movement-aligned spike files and does not itself preprocess the raw DANDI archive.

The two complete, commented training scripts used for the tutorial are available directly with the article:
- [DVS Gesture native NEST e-prop training script](https://edgarmanacorda.github.io/eprop_wired_paper/downloads/dvs_gesture_native_nest_eprop.py)
- [MC_Maze native NEST e-prop training script](https://edgarmanacorda.github.io/eprop_wired_paper/downloads/mc_maze_native_nest_eprop.py)
...
:::
For DVS Gesture, the training accuracy increases rapidly during the first training iterations and reaches approximately 0.8–0.9 near the end of training. When learning is disabled, train-evaluation and test accuracy remain high, indicating that the network learned the four-class gesture task and generalized reasonably well to held-out samples within this reduced four-class tutorial subset.
...
:::{seealso} Open the interactive 3D network browsers
For the clearest view and easiest manipulation of the 3D network, open each browser in a full-size page:

- [Open the DVS Gesture 3D network browser](https://eprop-wired-dashboard.onrender.com/network/dvs_gesture)
- [Open the MC_Maze 3D network browser](https://eprop-wired-dashboard.onrender.com/network/mc_maze)
The browsers allow the input, recurrent, and readout neurons to be displayed together. Separate top-k sliders control the number of visible `W_in_rec`, `W_rec_rec`, and `W_rec_out` connections.
:::
...
The two complete training scripts used to generate the tutorial results are provided as supporting files. The interactive diagnostic figures are embedded in the article, and the 3D network browsers are available through full-size external views. The original DVS Gesture and MC_Maze datasets remain referenced through their respective source publications and repositories.
+++
# Supporting files
