...
The training metrics are retrieved from the deployed Dash/Flask server through
its REST API. The client then builds the interactive Plotly figures used by the
Wired Paper.
"""
...
class EpropClient:
    def __init__(self, server_url: str | None = None, timeout: int = 600):
    ...
    def get_training_log(self, dataset: str) -> pd.DataFrame:
        """Retrieve a training log from the deployed dashboard API."""
    ...
    def training_figure(self, dataset: str) -> go.Figure:
        ...
        fig.update_layout(
            ...
            # Keep an explicit height so the live MyST/Jupyter rendering does
            # not collapse vertically when interactivity is activated.
            autosize=True,
            height=600,
            hovermode="closest",
